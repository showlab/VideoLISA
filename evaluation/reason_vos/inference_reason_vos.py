...
def parse_args(args):
    ...
    parser.add_argument("--vis_save_path", default="./results/VideoLISA-ReasonVOS/", type=str)
...
def main(args):
    ...
    # ---------------------------- read data ------------------------------------
    meta_exp_path = "ReasonVOS/meta_expressions.json"
    meta_exp = json.load(open(meta_exp_path, 'r'))["videos"]
    video_folder = "ReasonVOS/JPEGImages"
    ...
    for vid_key, exp_id, exp_text, is_sent in job_list_subset:
        ...
        if is_sent:
            prompt_template = "{sent}. Please output the segmentation masks."
            prompt = prompt_template.format(sent=ref_query)
        else:
            prompt_template = "Please segment the {class_name} in this scene."
